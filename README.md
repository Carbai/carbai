### Hi and welcome 👋

I'm a **Data Scientist** with a PhD in Computational Chemistry 👩🏻‍💻, based in Paris.

I work end-to-end on machine learning problems from framing ambiguous, open-ended
needs with stakeholders, through building and benchmarking models, to deploying and
monitoring them in production. I do my best work on problems that have no established method and have to be built
from the ground up.

Before industry, my PhD used quantum-chemical (DFT) and molecular dynamics simulations to
study chemistry at solid interfaces including applying unsupervised ML to simulation data.
That research background is where I learned how I still work: given a problem no one has
solved yet, figure it out and build something that works.

This is one of the simplest systems i've worked with during my PhD





<img align="left" src="https://github.com/Carbai/carbai_public/blob/master/clus.gif" width="400" style="margin-right: 25px;">  

Looking at how molecules interact and form, how cool is that?!

<br clear="left">

### Projects
[Pinky data science](https://pinky-ds.onrender.com)

### Short list for all the millenials who love lists:

- 🍺 On the organizing committee of [Pint of Science](https://pintofscience.it/) since 2018
- 👩‍🔬 Co-founded [ISPIRA](https://www.instagram.com/ispira_sns), a project inspiring young
  women into science — funded by Scuola Normale Superiore di Pisa, now running at national scale
- 🎤 Speaker mentoring at TED events
- 🌱 I’m constantly on a learning journey
- 💬 If you think I may be useful for anything please get in touch
- 📫 Get in touch at: baianocarmen@gmail.com
- ⚡ Fun fact: As chemist I only get dirty hands with experiments in my kitchen

### 📚 Data books I'd recommend

- **Weapons of Math Destruction** — Cathy O'Neil.
  How opaque algorithms encode bias and cause real harm at scale. Essential reading on the ethics of modelling.
  Easy to read, it is the one i'd usually recommend to people without technical experience. It would upset you though.
- **Invisible Women** — Caroline Criado Perez.
  How missing data erases women from decisions built on it. A powerful case study in what gaps in data actually cost.
  One of my favourites!! Real cases where products and policies were built and even called successes while quietly harming or ignoring women, because the data never accounted for them.
- **Calling Bullshit** — Carl T. Bergstrom & Jevin D. West.
  A practical field guide to spotting misleading data, charts, and statistical claims.
  This is more technical but the proof that statistics can be fun.
- **Naked Statistics** — Charles Wheelan.
  Statistical intuition made genuinely readable, the "why," not just the formulas.
- **NOW READING - Empire of AI** - Karen Hao 

![Snake animation](https://github.com/Carbai/carbai/blob/output/github-contribution-grid-snake.svg)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=carbai&layout=compact)](https://github.com/carbai/github-readme-stats)
